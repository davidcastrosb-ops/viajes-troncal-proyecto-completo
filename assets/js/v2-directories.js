(()=>{
  ...
  const visible=o=>{if(!o)return false;if(o.showWeb===false||String(o.showWeb||'').toLowerCase()==='no')return false;const e=o.expirationDate||o.expiresAt||o.fechaExpiracionWeb||'';return !(/^\d{4}-\d{2}-\d{2}$/.test(String(e))&&new Date(e+'T23:59:59')<new Date());};
  const hotelHref=(o,hotels)=>{const h=hotels.find(x=>x&&o.hotelId&&x.id===o.hotelId),s=h?.slug||hotelFallback[o.id]||'';return s?'/hotel-v2/'+encodeURIComponent(s)+'?oferta='+encodeURIComponent(o.id):'/oferta/'+encodeURIComponent(o.id);};
  ...
  function renderOffers(p){
    ...
    return [...groups.entries()].map(([id,rows])=>{const d=destinations.find(x=>x&&x.id===id),name=d?.name||rows[0]?.leadDestinationVerified||'Destino';rows.sort((a,b)=>(Number(a.ordenWeb)||999)-(Number(b.ordenWeb)||999));return '<section class="v2-dir-group"><div class="v2-dir-group-head"><div><span class="eyebrow">'+esc(d?.state||'Viajes disponibles')+'</span><h2>'+esc(name)+'</h2></div>'+(d?.slug?'<a href="/destino-v2/'+encodeURIComponent(d.slug)+'">Conocer '+esc(name)+' →</a>':'')+'</div><div class="v2-offer-grid">'+rows.map(o=>{const img=safe(o.image),href=hotelHref(o,hotels),dates=[o.travelStart?dateMx(o.travelStart):'',o.travelEnd?dateMx(o.travelEnd):''].filter(Boolean).join(' – '),dur=[o.days?o.days+' días':'',o.nights?o.nights+' noches':''].filter(Boolean).join(' · '),price=money(o.price||'');return '<article class="v2-offer-card"><a class="v2-offer-image" href="'+esc(href)+'">'+(img?'<img src="'+esc(img)+'" alt="'+esc(o.hotel||o.title||'Opción de viaje')+'" loading="lazy">':'<div>Conoce esta opción</div>')+'</a><div class="v2-offer-body"><div class="v2-offer-meta"><span>'+esc(o.plan||'Viaje')+'</span><small>'+esc(dates)+'</small></div><h3 translate="no" class="notranslate">'+esc(o.hotel||o.title||'Opción de viaje')+'</h3>'+(dur?'<p>'+esc(dur)+(o.occupancy?' · '+esc(o.occupancy):'')+'</p>':'')+'<div class="v2-offer-bottom"><div><small>'+esc(o.priceUnit||'Precio publicado')+'</small><strong>'+esc(price||'Consultar')+'</strong>'+(price?'<b> MXN</b>':'')+'</div><a href="'+esc(href)+'">Conocer hotel y oferta →</a></div></div></article>';}).join('')+'</div></section>';}).join('');
  }
