export default async function handler(req, res) {
  ...
  try {
    const response = await fetch(url, { headers: { "User-Agent": "Mozilla/5.0 ViajesTroncal/1.0" } });
    if (!response.ok) return res.status(response.status).json({ error: "No se pudo leer la promo" });
    ...
    return res.status(500).json({ error: "No se pudo procesar la promo", detail: String(error) });
  }
}