...
const FALLBACK_HOTEL = {
  'OF-PA-PVR-REV26-001': {id:'HOT-PVR-FRIENDLY-001',slug:'friendly-fun-vallarta'},
...
function publicNote(offer){const raw=clean(offer?.note||'');const fallback='Precio, disponibilidad y condiciones están sujetos a confirmación. Se reconfirman antes de cualquier pago o reservación.';if(!raw)return fallback;const internal=/(\bqa\b|pendiente de recaptura|se retir[oó]|\/promotion\/|url\s|enlace p[uú]blico del proveedor|nota interna|solo interno|uso interno)/i.test(raw);return internal?fallback:raw;}
...
export default async function handler(req,res){
  ...
  let payload;try{payload=await master();}catch(_){return res.status(503).send('No pudimos generar el PDF.');}
  ...
  const fallback=FALLBACK_HOTEL[offer.id]||{};
  ...
  const customerNote=publicNote(offer);
  const targetPath=hotelSlug?`/hotel-v2/${encodeURIComponent(hotelSlug)}?oferta=${encodeURIComponent(offer.id)}`:`/oferta/${encodeURIComponent(offer.id)}`;
  const canonical=`${requestProto(req)}://${requestHost(req)}${targetPath}`;

  const pdf=await PDFDocument.create();pdf.setTitle(`${title} | Trhoncal Travel`);pdf.setAuthor('Trhoncal Travel');pdf.setSubject('Promoción de viaje compartible');
  const regular=await pdf.embedFont(StandardFonts.Helvetica),bold=await pdf.embedFont(StandardFonts.HelveticaBold);
  const navy=rgb(.024,.247,.325),navyDark=rgb(.012,.18,.243),gold=rgb(.847,.686,.345),cream=rgb(.969,.957,.925),gray=rgb(.36,.43,.46),white=rgb(1,1,1);const W=595.28,H=841.89;

  const p1=pdf.addPage([W,H]);p1.drawRectangle({x:0,y:0,width:W,height:H,color:cream});p1.drawRectangle({x:0,y:H-92,width:W,height:92,color:navy});p1.drawText('TRHONCAL',{x:42,y:H-50,font:bold,size:23,color:white});p1.drawText('TRAVEL',{x:43,y:H-68,font:bold,size:10,color:gold});p1.drawText('Una opción para compartir',{x:385,y:H-55,font:regular,size:10,color:white});
  ...
  y=drawWrapped(p1,title,{x:42,y,font:bold,size:23,maxWidth:W-84,color:navyDark,lineHeight:27,maxLines:3})-8;if(hotel){p1.drawText(`Hotel: ${hotel}`,{x:42,y,font:bold,size:12,color:navy});y-=19;}p1.drawText(destinationName,{x:42,y,font:bold,size:11,color:gold});y-=24;
  ...
  if(price){p1.drawText(priceUnit(offer).toUpperCase(),{x:42,y,font:bold,size:9,color:gold});p1.drawText(`$${price}`,{x:42,y:y-30,font:bold,size:28,color:navy});p1.drawText('MXN',{x:185,y:y-25,font:bold,size:10,color:navy});y-=52;}
  drawWrapped(p1,customerNote,{x:42,y,font:regular,size:9.5,maxWidth:W-84,color:gray,lineHeight:14,maxLines:5});p1.drawText('Trhoncal Travel · WhatsApp 33 2933 5952 · viajestroncal@gmail.com',{x:42,y:28,font:regular,size:8.4,color:gray});

  const embedded=[];for(const url of gallery.slice(0,7)){const img=await embedImage(pdf,url);if(img)embedded.push(img);}
  if(embedded.length>=2){const p2=pdf.addPage([W,H]);p2.drawRectangle({x:0,y:0,width:W,height:H,color:cream});p2.drawText('CONOCE EL HOTEL',{x:42,y:H-58,font:bold,size:10,color:gold});p2.drawText(hotel||destinationName,{x:42,y:H-88,font:bold,size:28,color:navyDark});p2.drawText('Fotografías del hotel disponibles en esta versión.',{x:42,y:H-110,font:regular,size:10,color:gray});const slots=[[42,H-365,330,230],[384,H-245,169,110],[384,H-365,169,110],[42,H-540,245,150],[300,H-540,253,150]];embedded.slice(0,5).forEach((img,i)=>{const s=slots[i];cover(p2,img,s[0],s[1],s[2],s[3]);p2.drawRectangle({x:s[0],y:s[1],width:s[2],height:s[3],borderColor:white,borderWidth:2});});p2.drawText(`${embedded.length} fotografía${embedded.length===1?'':'s'} en la galería del hotel.`,{x:42,y:90,font:bold,size:10,color:navy});}
  ...
  const facts=[hotel?`Hotel: ${hotel}`:'',dates?`Fechas: ${dates}`:'',duration?`Estancia: ${duration}`:'',offer.plan?`Plan: ${clean(offer.plan)}`:'',offer.occupancy?`Viajeros: ${clean(offer.occupancy)}`:'',price?`${priceUnit(offer)}: $${price} MXN`:'' ].filter(Boolean);facts.forEach(f=>{p3.drawText('•',{x:48,y:py,font:bold,size:12,color:gold});py=drawWrapped(p3,f,{x:62,y:py,font:regular,size:11,maxWidth:485,color:navy,lineHeight:15,maxLines:2})-8;});
  ...
  py=Math.min(py-18,290);p3.drawText('CONDICIONES',{x:42,y:py,font:bold,size:10,color:gold});drawWrapped(p3,customerNote,{x:42,y:py-20,font:regular,size:10,maxWidth:340,color:gray,lineHeight:14,maxLines:8});
  const qrBuffer=await QRCode.toBuffer(canonical,{type:'png',width:220,margin:1,color:{dark:'#063F53',light:'#FFFFFF'}}),qr=await pdf.embedPng(qrBuffer),qs=120,qx=W-42-qs,qy=105;p3.drawRectangle({x:qx-8,y:qy-8,width:qs+16,height:qs+16,color:white,borderColor:gold,borderWidth:1});p3.drawImage(qr,{x:qx,y:qy,width:qs,height:qs});p3.drawText('Escanea para abrir esta opción',{x:qx+2,y:qy-24,font:bold,size:8,color:navy});p3.drawText('en Trhoncal Travel.',{x:qx+25,y:qy-37,font:regular,size:8,color:gray});
  ...
  const bytes=await pdf.save(),safeName=clean(hotel||destinationName).replace(/[^A-Za-z0-9áéíóúÁÉÍÓÚñÑ]+/g,'-').replace(/^-|-$/g,'')||'oferta';res.setHeader('Content-Type','application/pdf');res.setHeader('Content-Disposition',`attachment; filename="Trhoncal-Travel-${safeName}.pdf"`);res.setHeader('Cache-Control','no-store, max-age=0');res.setHeader('X-Robots-Tag','noindex,nofollow');return res.status(200).send(Buffer.from(bytes));
}
