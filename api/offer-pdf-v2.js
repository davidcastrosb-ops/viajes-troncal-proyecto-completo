...
export default async function handler(req,res){
  ...
  drawWrapped(p1,clean(offer.note||'Precio, disponibilidad y condiciones se reconfirman antes de reservar.'),{x:42,y,font:regular,size:9.5,maxWidth:W-84,color:gray,lineHeight:14,maxLines:5});p1.drawText('Trhoncal Travel · WhatsApp 33 2933 5952 · viajestroncal@gmail.com',{x:42,y:28,font:regular,size:8.4,color:gray});

  const embedded=[];for(const url of gallery.slice(0,7)){const img=await embedImage(pdf,url);if(img)embedded.push(img);}
  if(embedded.length>=2){const p2=pdf.addPage([W,H]);p2.drawRectangle({x:0,y:0,width:W,height:H,color:cream});p2.drawText('CONOCE EL HOTEL',{x:42,y:H-58,font:bold,size:10,color:gold});p2.drawText(hotel||destinationName,{x:42,y:H-88,font:bold,size:28,color:navyDark});p2.drawText('Fotografías aprobadas para uso web en la biblioteca del hotel.',{x:42,y:H-110,font:regular,size:10,color:gray});const slots=[[42,H-365,330,230],[384,H-245,169,110],[384,H-365,169,110],[42,H-540,245,150],[300,H-540,253,150]];embedded.slice(0,5).forEach((img,i)=>{const s=slots[i];cover(p2,img,s[0],s[1],s[2],s[3]);p2.drawRectangle({x:s[0],y:s[1],width:s[2],height:s[3],borderColor:white,borderWidth:2});});p2.drawText(`${embedded.length} fotografía${embedded.length===1?'':'s'} aprobada${embedded.length===1?'':'s'} en esta versión.`,{x:42,y:90,font:bold,size:10,color:navy});}
