...
async function loadMaster(){const sep=MASTER_ENDPOINT.includes('?')?'&':'?';const controller=new AbortController();const timeout=setTimeout(()=>controller.abort(),10000);try{const r=await fetch(`${MASTER_ENDPOINT}${sep}_ts=${Date.now()}`,{cache:'no-store',redirect:'follow',signal:controller.signal,headers:{'User-Agent':'TrhoncalTravel-HotelV2/1.1'}});if(!r.ok)throw new Error(`Master ${r.status}`);return await r.json();}finally{clearTimeout(timeout);}}

export default async function handler(req,res){
  if(req.method!=='GET'){res.setHeader('Allow','GET');return res.status(405).send('Method not allowed');}
  const slug=String(req.query.slug||'').trim(), hotel=HOTELS[slug];
  ...
  const offers=Array.isArray(payload.offers)?payload.offers:[],requested=String(req.query.oferta||'').trim(),validIds=new Set(hotel.offerIds);
  const offer=offers.find(o=>o&&validIds.has(o.id)&&(!requested||o.id===requested))||offers.find(o=>o&&validIds.has(o.id))||null;

  const images=hotel.images.filter(x=>safeUrl(x[0])),hero=images[0]||null,destinationName=offer?.leadDestinationVerified||hotel.destinationName;
  ...
  const quoteUrl=`/cotizar-v2?${quote.toString()}`,pdfUrl=offer?`/oferta-v2/${encodeURIComponent(offer.id)}.pdf`:'';
  const backUrl=`/destino-v2/${encodeURIComponent(hotel.destinationSlug)}#opciones`;
  const price=offer?.price?money(offer.price):'',dates=[offer?.travelStart?dateMx(offer.travelStart):'',offer?.travelEnd?dateMx(offer.travelEnd):''].filter(Boolean).join(' – ');
  ...
  const wa=`https://wa.me/?text=${encodeURIComponent(`${shareText}\n${previewUrl}`)}`,mail=`mailto:?subject=${encodeURIComponent(`Mira esta opción: ${hotel.name}`)}&body=${encodeURIComponent(`${shareText}\n\n${previewUrl}`)}`;
  const gallery=images.slice(0,5).map((img,i)=>`<button type="button" data-gallery-index="${i}"><img src="${esc(img[0])}" alt="${esc(img[1])}" loading="${i?'lazy':'eager'}">${i===4&&images.length>5?`<span class="hotel-v2-gallery-more">Ver ${images.length} fotos</span>`:''}</button>`).join('');
  const features=hotel.features.map(x=>`<div class="hotel-v2-feature">${esc(x)}</div>`).join(''),roomDetails=hotel.room.details.map(x=>`<li>${esc(x)}</li>`).join('');
  ...
  return res.status(200).send(`<!doctype html><html lang="es"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(offer?.title||hotel.name)} | Trhoncal Travel V2</title><meta name="robots" content="noindex,nofollow"><link rel="icon" type="image/svg+xml" href="/assets/images/trhoncal-travel-logo.svg"><link rel="stylesheet" href="/assets/css/styles.css"><link rel="stylesheet" href="/assets/css/brand-v2.css"><link rel="stylesheet" href="/assets/css/hotel-v2.css"><style>.hotel-v2-back-row{padding-top:18px}.hotel-v2-back{display:inline-flex;align-items:center;gap:6px;color:#063f53;text-decoration:none;font-weight:900;font-size:14px}.hotel-v2-back:hover{text-decoration:underline}@media(max-width:620px){.hotel-v2-back-row{padding-top:14px}.hotel-v2-back{font-size:13px}}</style></head><body class="hotel-v2-page">
...
<div class="container hotel-v2-back-row"><a class="hotel-v2-back" href="${esc(backUrl)}">← Volver a viajes para ${esc(destinationName)}</a></div>
<main class="hotel-v2-main">
<section class="hotel-v2-hero"><div class="container hotel-v2-hero-grid"><div class="hotel-v2-copy"><span class="eyebrow">${offer?'Promoción disponible':'Conoce el hotel'}</span><h1 translate="no" class="notranslate">${esc(offer?.title||hotel.name)}</h1><p class="hotel-v2-hotel" translate="no">Hotel: ${esc(hotel.name)}</p><p>${esc(hotel.description)}</p>${offer?`<div class="hotel-v2-tags">${dates?`<span>${esc(dates)}</span>`:''}${offer.days&&offer.nights?`<span>${offer.days} días · ${offer.nights} noches</span>`:''}${offer.occupancy?`<span>${esc(offer.occupancy)}</span>`:''}${offer.plan?`<span>${esc(offer.plan)}</span>`:''}</div><div class="hotel-v2-price-row">${price?`<div class="hotel-v2-price"><small>${esc(offer.priceUnit||'Precio publicado')}</small><strong>${esc(price)}</strong><span>MXN</span></div>`:''}<a class="btn btn-primary" href="${esc(quoteUrl)}">Quiero este viaje →</a></div>`:''}</div><div class="hotel-v2-hero-image">${hero?`<img src="${esc(hero[0])}" alt="${esc(hero[1])}">`:'<div class="offer-image-fallback">Galería del hotel en preparación</div>'}</div></div></section>
${offer?`<section class="hotel-v2-actions"><div class="container"><div class="hotel-v2-action-card"><strong>Guárdala, compártela o envíala a quien viaje contigo.</strong><div class="hotel-v2-action-buttons"><button class="hotel-v2-mini primary" type="button" data-native-share>Compartir promoción</button><a class="hotel-v2-mini gold" href="${esc(pdfUrl)}">Descargar PDF V2</a><a class="hotel-v2-mini" href="${esc(wa)}" target="_blank">WhatsApp</a><a class="hotel-v2-mini" href="${esc(mail)}">Correo</a><button class="hotel-v2-mini" type="button" data-copy>Copiar enlace</button></div></div></div></section>`:''}
...
<section class="hotel-v2-section" id="fotos"><div class="container"><span class="eyebrow">Conócelo sin salir de Trhoncal</span><h2>Fotos del hotel</h2>${gallery?`<div class="hotel-v2-gallery">${gallery}</div>`:'<div class="hotel-v2-gallery-empty">Galería en preparación.</div>'}</div></section>
<section class="hotel-v2-section" id="hotel"><div class="container hotel-v2-grid"><article class="hotel-v2-card"><span class="eyebrow">Lo esencial de esta promoción</span><h3>Tu opción</h3><ul class="hotel-v2-list">${essentials}</ul></article><article class="hotel-v2-card" id="servicios"><span class="eyebrow">Conoce el hotel</span><h3 translate="no">${esc(hotel.name)}</h3><p class="hotel-v2-section-copy">${esc(hotel.description)}</p><div class="hotel-v2-features">${features}</div></article></div></section>
<section class="hotel-v2-section" id="habitacion"><div class="container"><article class="hotel-v2-card"><span class="eyebrow">Tu habitación</span><h3>${esc(hotel.room.name)}</h3><ul class="hotel-v2-list">${roomDetails}</ul><p class="hotel-v2-room-note">La categoría exacta incluida se confirma con la promoción vigente.</p></article></div></section>
<section class="hotel-v2-section" id="ubicacion"><div class="container hotel-v2-map-grid"><div class="hotel-v2-map-copy"><span class="eyebrow">Ubicación</span><h2>${esc(destinationName)}</h2><p>${esc(hotel.address)}</p><a href="https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(hotel.address)}" target="_blank" rel="noopener noreferrer">Abrir mapa ↗</a></div><iframe class="hotel-v2-map-frame" loading="lazy" src="https://www.google.com/maps?q=${encodeURIComponent(hotel.address)}&output=embed" title="Mapa de ${esc(hotel.name)}"></iframe></div></section>
<section class="hotel-v2-final"><div class="container hotel-v2-final-card"><div><span class="eyebrow">Siguiente paso</span><h2>¿Te gustaría viajar con esta opción?</h2><p>Destino, fechas, hotel y promoción pasan precargados al formulario.</p></div><a class="btn btn-primary" href="${esc(quoteUrl)}">Solicitar esta opción →</a></div></section>
</main>
...
<script>
const GALLERY=${galleryData},lb=document.querySelector('.hotel-v2-lightbox'),stage=lb?.querySelector('.hotel-v2-lightbox-stage img'),strip=lb?.querySelector('.hotel-v2-lightbox-strip');let gi=0;
